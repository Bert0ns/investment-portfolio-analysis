...
export function HoveringTooltipOverlay({ hoveredBuilding }: HoveringTooltipOverlayProps) {
  return (
    <div className="absolute top-4 right-4 pointer-events-none bg-black/80 backdrop-blur-md px-4 py-3 rounded-xl border border-border shadow-lg flex flex-col gap-1 min-w-40">
      <p className="text-xs font-bold text-foreground truncate max-w-40">{hoveredBuilding.name}</p>
      <p className="text-[10px] text-muted-foreground uppercase">{hoveredBuilding.sector}</p>
      <p className="text-lg font-mono text-cyan-400 mt-1">{hoveredBuilding.value.toFixed(2)}%</p>
    </div>
  );
}
